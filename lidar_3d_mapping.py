...
def _ensure_projectairsim_on_path() -> None:
    ...
    # Реальная структура пакета в этом репозитории:
    # `client/python/projectairsim/src/projectairsim`
    projectairsim_src = repo_root / "client" / "python" / "projectairsim" / "src"
    if projectairsim_src.exists():
        sys.path.insert(0, str(projectairsim_src))
...
def _min_range_in_cone(points_xyz, az_min_rad: float, az_max_rad: float, max_range: float) -> float:
    ...
    Поддерживает "обернутые" конусы, пересекающие границу -pi/pi.
    Например: az_min=135°, az_max=-135° означает задний сектор вокруг 180°.
    """
    if points_xyz is None:
        return max_range
    ...
    def _wrap_to_pi(a: float) -> float:
        # переводим угол в диапазон [-pi, pi)
    ...
    az_min = _wrap_to_pi(float(az_min_rad))
    az_max = _wrap_to_pi(float(az_max_rad))
    # обычный случай
    if az_min <= az_max:
        mask = (az >= az_min) & (az <= az_max)
    else:
        # обернутый диапазон (пересекает -pi/pi)
        mask = (az >= az_min) | (az <= az_max)
    if not np.any(mask):
        return max_range
...
def _min_down_clearance(points_xyz, max_xy: float = 0.8, max_down: float = 999.0) -> float:
    """
    Минимальный вертикальный клиренс "вниз" в body frame.

    В этом проекте body frame задаётся как:
    - x: вперёд
    - y: вправо
    - z: вниз (положительное значение = ниже центра дрона)

    Функция возвращает min(z) среди точек, попадающих в цилиндр под дроном
    (по умолчанию радиус max_xy). Если подходящих точек нет — возвращает max_down.
    """
    if points_xyz is None:
        return max_down
    if getattr(points_xyz, "size", 0) == 0:
    ...
    # sample to keep it fast
    if pts.shape[0] > 6000:
        idx = np.random.choice(pts.shape[0], size=6000, replace=False)
        pts = pts[idx]
    ...
    # Берём точки строго ниже (z > 0), чтобы исключить шум вокруг центра/собственные точки
    mask = (xy <= float(max_xy)) & (z > 0.05)
    if not np.any(mask):
        return max_down
...
def _check_landing_gear_collision(points_xyz, landing_gear_height: float = 0.5, safety_margin: float = 1.0) -> bool:
    ...
    # Ищем препятствия примерно на высоте ножек: z около landing_gear_height.
    # Важно: safety_margin трактуем как горизонтальный радиус безопасности (не вертикальный),
    # иначе проверка будет срабатывать на любые стены "на уровне корпуса".
    gear_band = 0.35  # вертикальная "толщина" слоя вокруг высоты ножек (м)
    z_min = max(0.05, float(landing_gear_height) - gear_band)
    z_max = float(landing_gear_height) + gear_band

    z = pts[:, 2]
    mask_gear_level = (z >= z_min) & (z <= z_max)
    ...
    # Отсекаем точки слишком близко к центру (часто это шум/самолидар/артефакты)
    min_xy = 0.12
    collision_radius = float(safety_margin)

    close_mask = (xy_distances >= min_xy) & (xy_distances < collision_radius)
    # Требуем несколько точек, чтобы не реагировать на одиночный выброс
    min_points = 4
    return bool(int(np.count_nonzero(close_mask)) >= min_points)
...
async def _drive_to_waypoint_reactive(
    ...
    """
    Стабильная навигация к waypoint с плавным обходом препятствий.
    Использует сглаживание команд для предотвращения резких рывков.
    """
    t0 = time.time()
    target_z = z
    ...
    # Сглаживание команд
    smoothed_v_fwd = 0.0
    smoothed_v_right = 0.0
    smoothed_yaw_rate = 0.0
    smoothing_factor = 0.3
    
    # Режим обхода препятствия
    ...
    while True:
        ...
        front_min = _min_range_in_cone(pts, az_min_rad=-math.radians(25), az_max_rad=math.radians(25), max_range=999.0)
        left_min = _min_range_in_cone(pts, az_min_rad=math.radians(25), az_max_rad=math.radians(80), max_range=999.0)
        right_min = _min_range_in_cone(pts, az_min_rad=-math.radians(80), az_max_rad=-math.radians(25), max_range=999.0)

        # Базовая скорость к цели
        speed = min(cruise_speed, max(0.3, dist * 0.4))
        v_n = speed * (dn / max(dist, 0.1))
        v_e = speed * (de / max(dist, 0.1))
        v_fwd_desired, v_right_desired = _world_to_body(v_n, v_e, yaw)

        # Инициализируем команды
        v_fwd_cmd = v_fwd_desired
        v_right_cmd = v_right_desired
        yaw_rate_cmd = 0.0
        target_z = z

        # Порог обнаружения препятствия (уменьшен для более точного обхода)
        obstacle_threshold = min(avoid_dist * 0.5, 4.0)
        now = time.time()

        # Экстренная остановка при очень близком препятствии
        if front_min < 1.5:
        ...
        # Обнаружено препятствие впереди
        elif front_min < obstacle_threshold:
            if not avoiding_obstacle:
                avoiding_obstacle = True
                avoidance_direction = 1 if left_min > right_min else -1
                avoidance_start_time = now
            
            # Плавный обход
            obstacle_factor = max(0.2, front_min / obstacle_threshold)
            v_fwd_cmd = cruise_speed * obstacle_factor * 0.5
            v_right_cmd = avoidance_direction * cruise_speed * 0.5
            yaw_rate_cmd = avoidance_direction * max_yaw_rate * 0.4
        
        # Проверяем, обошли ли препятствие
        elif avoiding_obstacle:
            if now - avoidance_start_time > avoidance_timeout or front_min > obstacle_threshold * 1.5:
                avoiding_obstacle = False

        # Сглаживание команд
        smoothed_v_fwd = smoothed_v_fwd * smoothing_factor + v_fwd_cmd * (1.0 - smoothing_factor)
        smoothed_v_right = smoothed_v_right * smoothing_factor + v_right_cmd * (1.0 - smoothing_factor)
        smoothed_yaw_rate = smoothed_yaw_rate * smoothing_factor + yaw_rate_cmd * (1.0 - smoothing_factor)

        # Ограничиваем скорости
        smoothed_v_fwd = _clamp(smoothed_v_fwd, -cruise_speed, cruise_speed)
        smoothed_v_right = _clamp(smoothed_v_right, -cruise_speed * 0.6, cruise_speed * 0.6)
        smoothed_yaw_rate = _clamp(smoothed_yaw_rate, -max_yaw_rate * 0.6, max_yaw_rate * 0.6)

        # Отправляем команду
        cmd = await drone.move_by_velocity_body_frame_z_async(
            v_forward=smoothed_v_fwd,
            v_right=smoothed_v_right,
            z=target_z,
            duration=dt,
            yaw_is_rate=True,
            yaw=smoothed_yaw_rate,
        )
        await cmd
...
async def explore_area_reactive(
    ...
    Использует стабильную навигацию на основе ground truth с плавным обходом препятствий.
    """
    ...
    stuck_check_interval = 3.0  # проверяем застревание каждые 3 секунды
    stuck_position_history = []
    stuck_position_window = 5  # храним последние 5 позиций
    
    # Стабильная система навигации по waypoints вместо хаотичного исследования
    waypoint_idx = 0
    # NOTE: используем CLI-параметр --explore-lane-step (иначе траектория получается "в одну сторону" на узких складах)
    step_m = float(lane_step) if lane_step is not None else 5.0
    if not math.isfinite(step_m) or step_m <= 0:
        step_m = 5.0
    waypoints = _generate_exploration_waypoints(start_n, start_e, extent_n, extent_e, step=step_m)
    if not waypoints:
        waypoints = [(start_n + 10.0, start_e), (start_n + 10.0, start_e + 10.0), 
                     (start_n, start_e + 10.0), (start_n, start_e)]
    
    current_target_n, current_target_e = waypoints[waypoint_idx]
    
    # Фильтр для сглаживания команд (предотвращает резкие рывки)
    smoothed_v_fwd = 0.0
    smoothed_v_right = 0.0
    smoothed_yaw_rate = 0.0
    smoothing_factor = 0.3  # 0.0 = без сглаживания, 1.0 = максимальное сглаживание
    
    # Режим обхода препятствия
    avoiding_obstacle = False
    avoidance_direction = 0  # 1 = влево, -1 = вправо
    avoidance_start_time = 0.0
    avoidance_timeout = 5.0  # максимум 5 секунд обхода

    print(f"[explore] Starting stable exploration with {len(waypoints)} waypoints")
    ...
    while time.time() - t0 < total_timeout_sec:
        ...
        # Используем ground truth для стабильной навигации (избегаем нестабильности LIO-SLAM)
        pos = pose_msg.get("position", {}) if isinstance(pose_msg, dict) else {}
        ori = pose_msg.get("orientation", {}) if isinstance(pose_msg, dict) else {}
        
        cur_n = float(pos.get("x", 0.0))
        cur_e = float(pos.get("y", 0.0))
        cur_z = float(pos.get("z", z))
        yaw = _quat_to_yaw_rad(ori) if isinstance(ori, dict) else 0.0

        # Проверяем достижение текущей цели
        dn = current_target_n - cur_n
        de = current_target_e - cur_e
        dist_to_target = math.hypot(dn, de)
        
        if dist_to_target < arrive_tol:
            waypoint_idx = (waypoint_idx + 1) % len(waypoints)
            current_target_n, current_target_e = waypoints[waypoint_idx]
            print(f"[explore] Waypoint reached! Moving to waypoint {waypoint_idx + 1}/{len(waypoints)}")
            avoiding_obstacle = False  # сбрасываем режим обхода при достижении цели
        ...
        if now - last_stuck_check >= stuck_check_interval:
            ...
            if len(stuck_position_history) >= 3:
                ...
                if max_dist < 0.5:  # застряли
                    print(f"[explore] Stuck detected! Switching to next waypoint...")
                    waypoint_idx = (waypoint_idx + 1) % len(waypoints)
                    current_target_n, current_target_e = waypoints[waypoint_idx]
                    stuck_position_history.clear()
                    avoiding_obstacle = False
        ...
        # Базовая скорость к цели
        speed = min(cruise_speed, max(0.3, dist_to_target * 0.5))
        v_n_desired = speed * (dn / max(dist_to_target, 0.1))
        v_e_desired = speed * (de / max(dist_to_target, 0.1))
        v_fwd_desired, v_right_desired = _world_to_body(v_n_desired, v_e_desired, yaw)
        ...
        # Логика обхода препятствий (упрощенная и стабильная)
        obstacle_distance = avoid_dist * 0.6  # 60% от avoid_dist - порог обнаружения
        
        # Экстренная остановка при очень близком препятствии
        if front_min < 1.5:
            print(f"[explore] EMERGENCY: Obstacle at {front_min:.1f}m! Reversing...")
            v_fwd_cmd = -cruise_speed * 0.5
            v_right_cmd = 0.0
            yaw_rate_cmd = 0.0
            target_z = z - 0.5  # небольшой подъем
        
        # Обнаружено препятствие впереди
        elif front_min < obstacle_distance:
            if not avoiding_obstacle:
                # Начинаем обход - выбираем направление
                avoiding_obstacle = True
                avoidance_direction = 1 if left_min > right_min else -1
                avoidance_start_time = now
                print(f"[explore] Obstacle detected at {front_min:.1f}m. Avoiding {'left' if avoidance_direction > 0 else 'right'}")
            
            # Плавный обход: уменьшаем скорость вперед, добавляем боковое движение
            obstacle_factor = max(0.1, front_min / obstacle_distance)  # 0.1-1.0
            v_fwd_cmd = cruise_speed * obstacle_factor * 0.4  # замедляемся
            v_right_cmd = avoidance_direction * cruise_speed * 0.6  # боковое движение
            yaw_rate_cmd = avoidance_direction * max_yaw_rate * 0.5  # плавный поворот
        
        # Препятствие обойдено или таймаут обхода
        elif avoiding_obstacle:
            if now - avoidance_start_time > avoidance_timeout:
                avoiding_obstacle = False
                print("[explore] Avoidance timeout, resuming normal navigation")
            elif front_min > obstacle_distance * 1.5:  # чистый путь впереди
                avoiding_obstacle = False
                print("[explore] Path clear, resuming navigation to waypoint")

        # Сглаживание команд для предотвращения рывков
        smoothed_v_fwd = smoothed_v_fwd * smoothing_factor + v_fwd_cmd * (1.0 - smoothing_factor)
        smoothed_v_right = smoothed_v_right * smoothing_factor + v_right_cmd * (1.0 - smoothing_factor)
        smoothed_yaw_rate = smoothed_yaw_rate * smoothing_factor + yaw_rate_cmd * (1.0 - smoothing_factor)
        ...
        # Ограничиваем скорости
        smoothed_v_fwd = _clamp(smoothed_v_fwd, -cruise_speed, cruise_speed)
        smoothed_v_right = _clamp(smoothed_v_right, -cruise_speed * 0.7, cruise_speed * 0.7)
        smoothed_yaw_rate = _clamp(smoothed_yaw_rate, -max_yaw_rate * 0.7, max_yaw_rate * 0.7)
...
def _generate_exploration_waypoints(start_n: float, start_e: float, extent_n: float, extent_e: float, step: float = 5.0) -> List[Tuple[float, float]]:
    """
    Генерирует сетку waypoints для систематического исследования области.
    Использует паттерн "змейка" для эффективного покрытия.
    """
    waypoints = []
    
    # Определяем границы области
    n_min = start_n - abs(extent_n) / 2
    n_max = start_n + abs(extent_n) / 2
    e_min = start_e - abs(extent_e) / 2
    e_max = start_e + abs(extent_e) / 2
    
    # Если extent равен 0, используем небольшую область вокруг старта
    if extent_n == 0 or extent_e == 0:
        default_extent = 20.0
        n_min = start_n - default_extent / 2
        n_max = start_n + default_extent / 2
        e_min = start_e - default_extent / 2
        e_max = start_e + default_extent / 2
    
    # Генерируем waypoints по паттерну "змейка"
    e_current = e_min
    direction = 1  # 1 = движение к n_max, -1 = движение к n_min
    
    while e_current <= e_max:
        if direction == 1:
            waypoints.append((n_max, e_current))
        else:
            waypoints.append((n_min, e_current))
        
        e_current += step
        direction *= -1  # меняем направление для следующего прохода
    ...
    waypoints.append((start_n, start_e))
...
async def explore_map_systematic(
    ...
    while time.time() - t0 < total_timeout_sec:
        ...
        # ВНИЗ: используем реальный вертикальный клиренс под дроном,
        # а не азимутный конус (который может давать ложные срабатывания рядом со стенами).
        down_min = _min_down_clearance(pts, max_xy=0.8, max_down=999.0)
...
async def detect_map_bounds(
    ...
    """
    Определяет границы карты путем вращения дрона на месте и анализа данных LiDAR.
    
    Args:
        drone: Объект дрона
        lidar_latest: Последние данные лидара
        pose_latest: Последняя поза дрона
        z: Высота сканирования (NED, отрицательное = вверх)
        scan_duration: Длительность сканирования в секундах
        rotation_speed: Скорость вращения (рад/с)
    
    Returns:
        Tuple[float, float, dict]:
            - extent_n, extent_e: полуразмеры области относительно стартовой позиции (оценка)
            - bounds: словарь с абсолютными границами по N/E (квантили),
              чтобы можно было строить цели внутри реальных стен даже если старт не в центре.
    """
    print("[map_detection] Начинаем определение границ карты...")
    ...
    print(f"[map_detection] Стартовая позиция: ({start_n:.2f}, {start_e:.2f})")
    
    # Поднимаемся на заданную высоту
    print(f"[map_detection] Подъем на высоту {-z} метров для сканирования...")
    await drone.move_to_position_async(north=start_n, east=start_e, down=z, velocity=2.0)
    await asyncio.sleep(2.0)
    
    # Собираем точки для определения границ
    all_points_n = []
    all_points_e = []
    
    print(f"[map_detection] Вращение на месте для сканирования карты ({scan_duration} сек)...")
    start_time = time.time()
    yaw_start = 0.0
    ...
    if pose_msg is not None and isinstance(pose_msg, dict):
        ori = pose_msg.get("orientation", {})
        if ori:
            yaw_start = _quat_to_yaw_rad(ori)
    
    while time.time() - start_time < scan_duration:
        # Вращаем дрон вокруг своей оси
        current_yaw = yaw_start + rotation_speed * (time.time() - start_time)
        current_yaw = current_yaw % (2 * math.pi)
        
        # Получаем данные LiDAR
        pts, _ts = lidar_latest.snapshot()
        if pts is not None:
            try:
                import numpy as np
                if pts.shape[0] > 0:
                    # Трансформируем точки из body frame в world frame
                    pose_msg, _pose_ts = pose_latest.snapshot()
                    if pose_msg is not None and isinstance(pose_msg, dict):
                        pos = pose_msg.get("position", {})
                        ori = pose_msg.get("orientation", {})
                        if pos and ori:
                            pts_world = _transform_points_body_to_world(pts, pos, ori)
                            # Собираем координаты N и E
                            all_points_n.extend(pts_world[:, 0].tolist())
                            all_points_e.extend(pts_world[:, 1].tolist())
            except Exception as e:
                pass
        
        # Вращаем дрон
        await drone.move_by_velocity_body_frame_z_async(
            v_forward=0.0,
            v_right=0.0,
            z=z,
            duration=0.25,
    ...
    if len(all_points_n) > 0 and len(all_points_e) > 0:
        try:
            import numpy as np
            points_n = np.array(all_points_n)
            points_e = np.array(all_points_e)
            
            # Фильтруем выбросы (точки слишком далеко от центра)
            center_n = np.median(points_n)
            center_e = np.median(points_e)
            
            # Используем квантили для определения границ (игнорируем самые крайние выбросы)
            # 2/98 дает более полное покрытие карты, чем 5/95, но всё ещё устойчиво к шуму.
            min_n = np.percentile(points_n, 2)
            max_n = np.percentile(points_n, 98)
            min_e = np.percentile(points_e, 2)
            max_e = np.percentile(points_e, 98)
            
            # Вычисляем размеры относительно стартовой позиции
            extent_n = max(abs(max_n - start_n), abs(min_n - start_n)) * 1.2  # Добавляем 20% запас
            extent_e = max(abs(max_e - start_e), abs(min_e - start_e)) * 1.2
            
            # Минимальные размеры для безопасности
            extent_n = max(extent_n, 20.0)
            extent_e = max(extent_e, 20.0)
            
            print(f"[map_detection] Определены границы карты:")
            print(f"[map_detection]   North: от {min_n:.1f} до {max_n:.1f} (размер: {extent_n:.1f}м)")
            print(f"[map_detection]   East: от {min_e:.1f} до {max_e:.1f} (размер: {extent_e:.1f}м)")
            print(f"[map_detection]   Центр: ({center_n:.1f}, {center_e:.1f})")
            ...
            bounds = {
                ...
                "min_e": float(min_e),
                "max_e": float(max_e),
                "center_n": float(center_n),
                "center_e": float(center_e),
                "start_n": float(start_n),
                "start_e": float(start_e),
            }

            return extent_n, extent_e, bounds
        except Exception as e:
            print(f"[map_detection] Ошибка при обработке данных: {e}")
    
    # Если не удалось определить границы, используем значения по умолчанию
    print("[map_detection] Не удалось определить границы карты, используем значения по умолчанию")
    extent_n = 50.0
    extent_e = 50.0
    bounds = {
        "min_n": float(start_n - extent_n),
        "max_n": float(start_n + extent_n),
        "min_e": float(start_e - extent_e),
        "max_e": float(start_e + extent_e),
        "center_n": float(start_n),
        "center_e": float(start_e),
        "start_n": float(start_n),
        "start_e": float(start_e),
    }
    return extent_n, extent_e, bounds
...
async def explore_map_with_path_visualization(
    drone: Drone,
    world: World,
    lidar_latest: LidarLatest,
    pose_latest: PoseLatest,
    imu_latest: ImuLatest,
    lio_slam: SimpleLIO,
    extent_n: Optional[float],
    extent_e: Optional[float],
    lane_step_m: float,
    z: float,
    cruise_speed: float,
    dt: float,
    ...
    """
    Алгоритм облета карты с визуализацией маршрута красной линией.
    Дрон поднимается на заданную высоту и полностью облетает карту, избегая препятствий.
    Маршрут отображается красной линией в симуляции.
    
    Args:
        drone: Объект дрона
        world: Объект World для визуализации маршрута
        lidar_latest: Последние данные лидара
        pose_latest: Последняя поза дрона
        imu_latest: Последние данные IMU
        lio_slam: Объект LIO-SLAM для точной навигации
        extent_n: Размер области по North (м), None для автоматического определения
        extent_e: Размер области по East (м), None для автоматического определения
        z: Высота полета (NED, отрицательное = вверх, например -5.0 для 5 метров)
        cruise_speed: Крейсерская скорость (м/с)
        dt: Шаг управления (сек)
        arrive_tol: Допуск достижения точки (м)
        avoid_dist: Дистанция срабатывания уклонения (м)
        ...
        total_timeout_sec: Общий таймаут исследования (сек)
    """
    print("[path_visualization] Начинаем облет карты с визуализацией маршрута")
    
    # Автоматическое определение границ карты, если не заданы
    detected_bounds: Optional[dict] = None
    if extent_n is None or extent_e is None:
        print("[path_visualization] Границы карты не заданы, определяем автоматически...")
        detected_extent_n, detected_extent_e, detected_bounds = await detect_map_bounds(
            drone=drone,
            lidar_latest=lidar_latest,
            pose_latest=pose_latest,
            z=z,
        ...
        if extent_n is None:
            extent_n = detected_extent_n
        if extent_e is None:
            extent_e = detected_extent_e
        print(f"[path_visualization] Используем определенные границы: extent_n={extent_n:.1f}м, extent_e={extent_e:.1f}м")
    else:
        print(f"[path_visualization] Используем заданные границы: extent_n={extent_n:.1f}м, extent_e={extent_e:.1f}м")
    ...
    print(f"[path_visualization] Стартовая позиция: ({start_n:.2f}, {start_e:.2f}, {start_z:.2f})")
    
    # Поднимаемся на заданную высоту (5 метров)
    print(f"[path_visualization] Подъем на высоту {-z} метров...")
    await drone.move_to_position_async(north=start_n, east=start_e, down=z, velocity=2.0)
    await asyncio.sleep(2.0)  # Ждем стабилизации
    
    # История позиций для визуализации маршрута
    path_history = []
    last_path_update = time.time()
    path_update_interval = 0.5  # Обновляем визуализацию каждые 0.5 секунды
    ...
    # План покрытия области (serpentine / "газонокосилка"):
    # вместо бесконечного "лететь вперёд" по одному курсу мы генерируем цели в пределах границ
    # и каждый тик направляемся к текущей цели, сохраняя реактивное избегание препятствий.
    #
    # extent_n/extent_e: в остальных режимах CLI трактуются как "полный размер" области.
    # Здесь держим ту же семантику: используем [start - extent/2, start + extent/2] если не было авто-детекта стен.
    # Для полёта "между стеллажами" и вдоль стен делаем отступ и буфер у стен настраиваемыми через avoid_dist,
    # но без жёсткого минимума 2.5м, который часто заставляет лететь в одну линию.
    coverage_margin_m = max(1.0, avoid_dist * 0.25)
    if lane_step_m is not None and float(lane_step_m) > 0:
        coverage_step_m = float(lane_step_m)
    else:
        coverage_step_m = max(2.0, min(6.0, avoid_dist * 0.9))
    coverage_step_m = max(1.5, min(12.0, coverage_step_m))

    # Если старт не в центре, симметричная область может увести цели "за стену".
    # Поэтому, при наличии абсолютных границ из `detect_map_bounds`, используем их.
    if detected_bounds and isinstance(detected_bounds, dict):
        n_min_raw = float(detected_bounds.get("min_n", start_n - float(extent_n)))
        n_max_raw = float(detected_bounds.get("max_n", start_n + float(extent_n)))
        e_min_raw = float(detected_bounds.get("min_e", start_e - float(extent_e)))
        e_max_raw = float(detected_bounds.get("max_e", start_e + float(extent_e)))
    else:
        # пользовательские extent_* (CLI) интерпретируем как полный размер (как в explore/systematic)
        n_half = abs(float(extent_n)) * 0.5
        e_half = abs(float(extent_e)) * 0.5
        n_min_raw = start_n - n_half
        n_max_raw = start_n + n_half
        e_min_raw = start_e - e_half
        e_max_raw = start_e + e_half

    n_min = n_min_raw + coverage_margin_m
    n_max = n_max_raw - coverage_margin_m
    e_min = e_min_raw + coverage_margin_m
    e_max = e_max_raw - coverage_margin_m
    ...
    coverage_targets: List[Tuple[float, float]] = []
    # Сканируем "полосами" по East, а внутри полосы идём от n_min до n_max (и обратно)
    e = e_min
    forward = True
    while e <= e_max + 1e-6:
        if forward:
            coverage_targets.append((n_min, e))
            coverage_targets.append((n_max, e))
        else:
            coverage_targets.append((n_max, e))
            coverage_targets.append((n_min, e))
        forward = not forward
    ...
    # Если область слишком мала, хотя бы одна цель (чтобы не делить на ноль ниже)
    if not coverage_targets:
        coverage_targets = [(start_n, start_e)]
    
    target_idx = 0
    print(f"[path_visualization] План покрытия: {len(coverage_targets)} целей, шаг {coverage_step_m:.1f}м, отступ {coverage_margin_m:.1f}м")
    print(f"[path_visualization] Границы покрытия: N[{n_min:.1f}, {n_max:.1f}] E[{e_min:.1f}, {e_max:.1f}]")
    
    print("[path_visualization] Начинаем реактивное исследование с визуализацией маршрута")
    ...
    while time.time() - t0 < total_timeout_sec:
        ...
        # Добавляем текущую позицию в историю маршрута
        path_history.append([cur_n, cur_e, cur_z])
        
        # Обновляем визуализацию маршрута периодически
        now = time.time()
        if now - last_path_update >= path_update_interval:
            if len(path_history) >= 2:
                # Рисуем красную линию маршрута
                # Используем последние точки для плавной линии
                points_to_draw = path_history[-100:] if len(path_history) > 100 else path_history
                color_rgba = [1.0, 0.0, 0.0, 1.0]  # Красный цвет
                thickness = 3.0
                duration = path_update_interval * 2  # Длительность немного больше интервала обновления
                is_persistent = True  # Линия остается видимой
                ...
                try:
                    ...
                    print(f"[path_visualization] Ошибка при рисовании линии: {e}")
        ...
        # Проверка на застревание (более чувствительная)
        ...
        if now - last_stuck_check >= stuck_check_interval:
            ...
            if len(stuck_position_history) >= 3:
                ...
                # Более чувствительное обнаружение застревания (0.5м вместо 1.0м)
                if max_dist < 0.5 and speed < 0.5:
                    stuck_detected = True
                    print(f"[path_visualization] ЗАСТРЯЛ! (движение < 0.5м за последние проверки)")
            
            # Проверка на угол (препятствия близко с нескольких сторон)
            pts_check, _ = lidar_latest.snapshot()
            if pts_check is not None:
                # Используем широкие конусы для раннего обнаружения
                front_check = _min_range_in_cone(pts_check, az_min_rad=-math.radians(45), az_max_rad=math.radians(45), max_range=999.0)
                left_check = _min_range_in_cone(pts_check, az_min_rad=math.radians(20), az_max_rad=math.radians(90), max_range=999.0)
                right_check = _min_range_in_cone(pts_check, az_min_rad=-math.radians(90), az_max_rad=-math.radians(20), max_range=999.0)
                
                # Безопасные дистанции для обнаружения угла
                CORNER_DETECT_DIST = 1.0  # метров
                
                # Угол: препятствия близко спереди и с одной стороны
                if front_check < CORNER_DETECT_DIST and (left_check < CORNER_DETECT_DIST or right_check < CORNER_DETECT_DIST):
                    corner_detected = True
                    stuck_detected = True
                    print(f"[path_visualization] !!! ОБНАРУЖЕН УГОЛ !!! спереди: {front_check:.2f}м, слева: {left_check:.2f}м, справа: {right_check:.2f}м")
            
            # Агрессивный маневр выхода при застревании
            if stuck_detected:
                print(f"[path_visualization] !!! ВЫПОЛНЯЮ ЭКСТРЕННЫЙ МАНЕВР ВЫХОДА ИЗ ЗАСТРЕВАНИЯ !!!")
                
                pts_stuck, _ = lidar_latest.snapshot()
                if pts_stuck is not None:
                    front_stuck = _min_range_in_cone(pts_stuck, az_min_rad=-math.radians(45), az_max_rad=math.radians(45), max_range=999.0)
                    left_stuck = _min_range_in_cone(pts_stuck, az_min_rad=math.radians(20), az_max_rad=math.radians(90), max_range=999.0)
                    right_stuck = _min_range_in_cone(pts_stuck, az_min_rad=-math.radians(90), az_max_rad=-math.radians(20), max_range=999.0)
                    # Задний сектор: диапазон пересекает -pi/pi, используем az_max=-135°
                    back_stuck = _min_range_in_cone(pts_stuck, az_min_rad=math.radians(135), az_max_rad=-math.radians(135), max_range=999.0)
                    
                    # Определяем лучшее направление для отхода
                    best_side_stuck = "left" if left_stuck > right_stuck else "right"
                    best_clearance_stuck = max(left_stuck, right_stuck)
                    
                    # Агрессивный отход: назад и в сторону с большим зазором, подъем
                    turn_sign = 1.0 if best_side_stuck == "left" else -1.0
                    
                    # Если сзади есть место - отходим быстро назад
                    if back_stuck > 5.0:
                        # Быстро, но безопаснее (меньше шанс "впечататься" в стену при ошибке оценки)
                        v_fwd_escape = -cruise_speed * 1.4
                        v_right_escape = turn_sign * cruise_speed * 1.0
                        yaw_rate_escape = turn_sign * max_yaw_rate
                        target_z_escape = z - 4.0  # Значительный подъем
                        print(f"[path_visualization] ЭКСТРЕННЫЙ отход назад и в {best_side_stuck}, подъем на 4м")
                    else:
                        # Если сзади тоже препятствие - ВВЕРХ и в сторону
                        v_fwd_escape = -cruise_speed * 0.6  # Осторожно назад
                        v_right_escape = turn_sign * cruise_speed * 1.2
                        yaw_rate_escape = turn_sign * max_yaw_rate
                        target_z_escape = z - 6.0  # ОЧЕНЬ высокий подъем для выхода
                        print(f"[path_visualization] ОКРУЖЕН! Экстренный подъем на 6м и отход в {best_side_stuck}")
                    
                    # Выполняем маневр выхода немедленно
                    cmd_escape = await drone.move_by_velocity_body_frame_z_async(
                        v_forward=v_fwd_escape,
                        v_right=v_right_escape,
                        z=target_z_escape,
                        duration=1.4,
                        yaw_is_rate=True,
                        yaw=yaw_rate_escape,
                    ...
                    await cmd_escape
                    await asyncio.sleep(0.3)  # Пауза после маневра
                    
                    # Меняем предпочтительное направление для избежания повторного застревания
                    # Поворот на 180 градусов от текущего направления + случайное отклонение
                    preferred_heading_rad = (preferred_heading_rad + math.pi + (random.random() - 0.5) * math.pi / 2) % (2 * math.pi)
                    last_direction_change = now
                    print(f"[path_visualization] Новое направление после выхода: {math.degrees(preferred_heading_rad):.1f}°")
                    
                    # Также переключаемся на следующую цель покрытия, чтобы не "упираться" в тот же карман
                    if coverage_targets:
                        target_idx = (target_idx + 1) % len(coverage_targets)
                    
                    # Очищаем историю позиций чтобы не срабатывало сразу снова
                    stuck_position_history.clear()
                    
                    # Пропускаем обычную логику управления в этом цикле
                    last_stuck_check = now
                    continue
            
            last_stuck_check = now
        
        # ====== Навигация по плану покрытия (вместо случайного preferred_heading) ======
        # Выбираем текущую цель и направляемся к ней (реактивное избегание остаётся ниже)
        if coverage_targets:
            target_n, target_e = coverage_targets[target_idx]
            # Подстраховка: не даем цели выходить за границы покрытия из-за накопленной ошибки
            target_n = _clamp(float(target_n), n_min, n_max)
            target_e = _clamp(float(target_e), e_min, e_max)
            dist_to_target = math.hypot(target_n - cur_n, target_e - cur_e)
            # Немного более мягкий порог, чтобы дрон не "зацикливался" вокруг точки
            if dist_to_target <= max(arrive_tol, 1.5):
                target_idx = (target_idx + 1) % len(coverage_targets)
                target_n, target_e = coverage_targets[target_idx]
                target_n = _clamp(float(target_n), n_min, n_max)
                target_e = _clamp(float(target_e), e_min, e_max)
            ...
            preferred_heading_rad = math.atan2((target_e - cur_e), (target_n - cur_n))
        
        pts, _ts = lidar_latest.snapshot()
        
        # ============= БЕЗОПАСНЫЕ ДИСТАНЦИИ =============
        # Критическая дистанция - НЕМЕДЛЕННАЯ ОСТАНОВКА
        CRITICAL_DIST = 3.0  # метров - если ближе, полная остановка и отход
        # Опасная дистанция - агрессивный маневр
        DANGER_DIST = 5.0  # метров - активное уклонение
        # Предупреждающая дистанция - начало маневра
        WARNING_DIST = max(avoid_dist, 8.0)  # метров - начинаем реагировать
        
        # Проверка на опасность столкновения ножек с препятствиями
        gear_collision_danger = _check_landing_gear_collision(pts, landing_gear_height=0.5, safety_margin=2.0)
        
        # Проверяем препятствия в разных направлениях (РАСШИРЕННЫЕ КОНУСЫ для лучшего обнаружения)
        # Широкий передний конус для раннего обнаружения
        front_min = _min_range_in_cone(pts, az_min_rad=-math.radians(45), az_max_rad=math.radians(45), max_range=999.0)
        # Узкий передний конус для точного определения
        front_narrow = _min_range_in_cone(pts, az_min_rad=-math.radians(15), az_max_rad=math.radians(15), max_range=999.0)
        left_min = _min_range_in_cone(pts, az_min_rad=math.radians(20), az_max_rad=math.radians(90), max_range=999.0)
        right_min = _min_range_in_cone(pts, az_min_rad=-math.radians(90), az_max_rad=-math.radians(20), max_range=999.0)
        # Задний сектор: диапазон пересекает -pi/pi, используем az_max=-135°
        back_min = _min_range_in_cone(pts, az_min_rad=math.radians(135), az_max_rad=-math.radians(135), max_range=999.0)
        # ВНИЗ: используем реальный вертикальный клиренс под дроном,
        # а не азимутный конус (который может давать ложные срабатывания рядом со стенами).
        down_min = _min_down_clearance(pts, max_xy=0.8, max_down=999.0)
        
        # Минимальное расстояние до любого препятствия спереди
        min_front = min(front_min, front_narrow)
        
        # ============= КРИТИЧЕСКАЯ ОПАСНОСТЬ - НЕМЕДЛЕННАЯ ОСТАНОВКА =============
        if min_front < CRITICAL_DIST:
            print(f"[path_visualization] !!! КРИТИЧЕСКАЯ ОПАСНОСТЬ ({min_front:.2f}м < {CRITICAL_DIST}м) !!! ПОЛНАЯ ОСТАНОВКА И ОТХОД!")
            best_side_crit = "left" if left_min > right_min else "right"
            turn_sign_crit = 1.0 if best_side_crit == "left" else -1.0
            
            # ПОЛНАЯ ОСТАНОВКА + БЫСТРЫЙ ОТХОД НАЗАД
            v_fwd_cmd = -cruise_speed * 2.0  # Максимально быстро назад
            v_right_cmd = turn_sign_crit * cruise_speed * 1.5  # Быстро в сторону
            yaw_rate_cmd = turn_sign_crit * max_yaw_rate  # Максимальный поворот
            target_z = z - 3.0  # Значительный подъем
            
            cmd = await drone.move_by_velocity_body_frame_z_async(
                v_forward=v_fwd_cmd,
                v_right=v_right_cmd,
                z=target_z,
                duration=1.5,  # Достаточно долго для безопасного отхода
                yaw_is_rate=True,
                yaw=yaw_rate_cmd,
            )
            await cmd
            await asyncio.sleep(0.2)
            print(f"[path_visualization] Экстренный отход выполнен, продолжаем в направлении {best_side_crit}")
            continue
        
        # ============= ОПАСНОСТЬ СТОЛКНОВЕНИЯ НОЖКАМИ =============
        if gear_collision_danger or (down_min < DANGER_DIST * 0.5):
            print(f"[path_visualization] ОПАСНОСТЬ: Препятствие на уровне ножек! Экстренный подъем...")
            v_fwd_cmd = -cruise_speed * 1.0
            target_z = z - 2.5  # Значительный подъем
            v_right_cmd = 0.0
            yaw_rate_cmd = 0.0
            if back_min < DANGER_DIST:
                turn_sign = 1.0 if left_min > right_min else -1.0
                yaw_rate_cmd = turn_sign * max_yaw_rate
                v_right_cmd = turn_sign * cruise_speed * 0.8
        ...
        # ============= ОПАСНАЯ ЗОНА - АГРЕССИВНЫЙ МАНЕВР =============
        if min_front < DANGER_DIST:
            print(f"[path_visualization] ОПАСНАЯ ЗОНА ({min_front:.2f}м < {DANGER_DIST}м)! Агрессивный маневр уклонения!")
            best_side = "left" if left_min > right_min else "right"
            turn_sign = 1.0 if best_side == "left" else -1.0
            best_clearance = max(left_min, right_min)
            
            # Отход назад + боковое движение + подъем
            v_fwd_cmd = -cruise_speed * 1.2
            v_right_cmd = turn_sign * cruise_speed * 1.0
            yaw_rate_cmd = turn_sign * max_yaw_rate * 0.9
            target_z = z - 2.0
            
            print(f"[path_visualization] Маневр: отход назад, движение в {best_side} (зазор: {best_clearance:.1f}м), подъем")
        ...
        if min_front < WARNING_DIST and (left_min < WARNING_DIST * 0.7 or right_min < WARNING_DIST * 0.7):
            approaching_corner = True
            print(f"[path_visualization] ПРИБЛИЖЕНИЕ К УГЛУ! спереди: {min_front:.2f}м, слева: {left_min:.2f}м, справа: {right_min:.2f}м")
        
        if approaching_corner:
            best_side = "left" if left_min > right_min else "right"
            turn_sign = 1.0 if best_side == "left" else -1.0
            
            # Остановка движения вперед + поворот в сторону с зазором
            v_fwd_cmd = -cruise_speed * 0.5  # Медленно назад
            v_right_cmd = turn_sign * cruise_speed * 0.8
            yaw_rate_cmd = turn_sign * max_yaw_rate * 0.8
            target_z = z - 1.5
            
            print(f"[path_visualization] Обход угла: движение в {best_side}, подъем")
        else:
            # ============= НОРМАЛЬНОЕ ДВИЖЕНИЕ С ПРОВЕРКОЙ ПРЕПЯТСТВИЙ =============
            # Желаемое движение к цели: скорость уменьшаем при приближении,
            # чтобы не "упираться" в стену/точку и проще выходить из карманов.
            if coverage_targets:
                dn_t = target_n - cur_n
                de_t = target_e - cur_e
                dist_t = max(0.0, math.hypot(dn_t, de_t))
                speed_t = min(cruise_speed, max(0.25, dist_t * 0.35))
                desired_v_n = speed_t * (dn_t / max(dist_t, 0.1))
                desired_v_e = speed_t * (de_t / max(dist_t, 0.1))
            else:
                desired_v_n = cruise_speed * math.cos(preferred_heading_rad)
                desired_v_e = cruise_speed * math.sin(preferred_heading_rad)

            # Мягкое "выталкивание" внутрь границ покрытия.
            # На складах с узкими проходами слишком большой минимальный буфер (2.5м) приводит к движению "в одну сторону".
            boundary_buf = max(1.2, avoid_dist * 0.55)
            if cur_n < n_min + boundary_buf:
                desired_v_n += (n_min + boundary_buf - cur_n) / boundary_buf * cruise_speed * 0.7
            elif cur_n > n_max - boundary_buf:
                desired_v_n -= (cur_n - (n_max - boundary_buf)) / boundary_buf * cruise_speed * 0.7
            if cur_e < e_min + boundary_buf:
                desired_v_e += (e_min + boundary_buf - cur_e) / boundary_buf * cruise_speed * 0.7
            elif cur_e > e_max - boundary_buf:
                desired_v_e -= (cur_e - (e_max - boundary_buf)) / boundary_buf * cruise_speed * 0.7
            
            # Конвертируем в body frame
            v_fwd_desired, v_right_desired = _world_to_body(desired_v_n, desired_v_e, yaw)
            
            # Отталкивающая сила от препятствий.
            # Не "раздуваем" influence/max_repulse: в узких проходах это мешает пролетать между стеллажами.
            rep_fwd, rep_right = _repulsive_velocity_xy(
                pts,
                influence_dist=influence_dist,
                max_repulse=max_repulse,
                landing_gear_height=0.5,
            )
            ...
            # Проверка препятствий в зоне предупреждения
            if min_front < WARNING_DIST:
                obstacle_ratio = max(0.0, min_front / max(WARNING_DIST, 0.1))
                best_side = "left" if left_min > right_min else "right"
                turn_sign = 1.0 if best_side == "left" else -1.0
                best_clearance = max(left_min, right_min)
                
                # Снижаем скорость вперед пропорционально близости препятствия
                speed_reduction = obstacle_ratio ** 2  # Квадратичное снижение - более агрессивное
                v_fwd_cmd = v_fwd_cmd * speed_reduction * 0.5
                
                # Добавляем боковое движение для обхода
                v_right_cmd = turn_sign * cruise_speed * (1.0 - obstacle_ratio) * 0.8
                yaw_rate_cmd = turn_sign * max_yaw_rate * (1.0 - obstacle_ratio) * 0.7
                
                # Небольшой подъем при обходе
                target_z = z - (1.0 - obstacle_ratio) * 1.5
                
                print(f"[path_visualization] Обход препятствия ({min_front:.1f}м): {best_side}, скорость {speed_reduction*100:.0f}%, зазор: {best_clearance:.1f}м")
            
            # Проверка препятствий снизу
            if down_min < WARNING_DIST * 0.8:
                target_z = min(target_z, z - 1.5)
        
        # Ограничиваем скорости (увеличенные лимиты для экстренных маневров)
        max_escape_speed = cruise_speed * 2.0
        v_fwd_cmd = _clamp(v_fwd_cmd, -max_escape_speed, cruise_speed)
        v_right_cmd = _clamp(v_right_cmd, -max_escape_speed, max_escape_speed)
        yaw_rate_cmd = _clamp(yaw_rate_cmd, -max_yaw_rate * 1.2, max_yaw_rate * 1.2)
        
        # Финальная проверка безопасности перед отправкой команды
        # Если есть препятствие ближе 1 метра - НЕ двигаемся вперед
        if min_front < 1.0 and v_fwd_cmd > 0:
            v_fwd_cmd = 0.0  # Полная блокировка движения вперед
    ...
    print("[path_visualization] Облет завершен. Возвращаемся к старту...")
    
    # Финальная визуализация всего маршрута
    if len(path_history) >= 2:
        print(f"[path_visualization] Рисуем финальный маршрут из {len(path_history)} точек...")
        color_rgba = [1.0, 0.0, 0.0, 1.0]  # Красный цвет
        thickness = 5.0
        duration = 3600.0  # Линия остается видимой 1 час
        is_persistent = True
        ...
        try:
            ...
            print("[path_visualization] Маршрут успешно визуализирован красной линией")
        except Exception as e:
            print(f"[path_visualization] Ошибка при финальной визуализации маршрута: {e}")
    ...
    print("[path_visualization] Миссия завершена. Красная линия маршрута остается видимой в симуляции.")
...
async def main():
    ...
    parser.add_argument("--mission", default="slam", choices=["explore", "square", "shelves", "slam", "systematic", "path_viz"])
    ...
    parser.add_argument("--explore-extent-n", type=float, default=0.0, help="Размер области по North (м) от старта. 0 = автоматическое определение.")
    parser.add_argument("--explore-extent-e", type=float, default=0.0, help="Размер области по East (м) от старта. 0 = автоматическое определение.")
    ...
    parser.add_argument("--ctrl-dt", type=float, default=0.15, help="Шаг управления (сек), меньше = более реактивно.")
    parser.add_argument("--avoid-dist", type=float, default=4.0, help="Дистанция срабатывания уклонения (м). Оптимизировано для плавного обхода.")
    parser.add_argument("--influence-dist", type=float, default=6.0, help="Радиус влияния для отталкивания (м). Оптимизировано для стабильного полета.")
    parser.add_argument("--max-repulse", type=float, default=1.5, help="Максимальная отталкивающая скорость (м/с).")
    parser.add_argument("--max-yaw-rate", type=float, default=0.6, help="Максимальная скорость рыскания (рад/с). Уменьшено для плавности.")
    ...
    try:
        ...
        if args.mission != "slam":
            if args.mission == "square":
            ...
            elif args.mission == "path_viz":
                print("[path_viz] starting map exploration with path visualization (red line)")
                # Инициализация LIO-SLAM с начальной позицией
                pose_msg, _ts = pose_latest.snapshot()
                if pose_msg is not None and isinstance(pose_msg, dict):
                    pos = pose_msg.get("position", {})
                ...
                # Для path_viz используем автоматическое определение границ, если не заданы явно
                extent_n = args.explore_extent_n if args.explore_extent_n > 0 else None
                extent_e = args.explore_extent_e if args.explore_extent_e > 0 else None
                
                await explore_map_with_path_visualization(
                    drone=drone,
                    world=world,
                    lidar_latest=lidar_latest,
                    pose_latest=pose_latest,
                    ...
                    extent_n=extent_n,
                    extent_e=extent_e,
                    lane_step_m=args.explore_lane_step,
                    z=args.height,
                    cruise_speed=args.velocity,
                    dt=args.ctrl_dt,
                    arrive_tol=args.arrive_tol,
                ...
                await explore_area_reactive(
                    ...
                    extent_n=args.explore_extent_n,
                    extent_e=args.explore_extent_e,
                    lane_step=args.explore_lane_step,
                    z=args.height,
                    cruise_speed=args.velocity,
                    dt=args.ctrl_dt,
                    arrive_tol=args.arrive_tol,
